...
const SearchManufacturer = ({ manufacturer, setManufacturer }: SearchManufacturerProps) => {
  ...
  const handleManufacturerChange = (selectedManufacturer: string) => {
    ...
    setQuery(selectedManufacturer)
  }

  return (
    <div className="search-manufacturer">
      <Combobox value={manufacturer} onChange={handleManufacturerChange} displayValue={(manufacturer: string) => manufacturer}>
        <div className="relative w-full">
          ...
          <Transition
            ...
            leaveTo="opacity-0"
            afterLeave={() => setQuery(manufacturer || "")}
          >
            <Combobox.Options>
              {filteredManufacturers.length === 0 && query !== "" ? (
                <div className="search-manufacturer__option">No results found</div>
              ) : (
